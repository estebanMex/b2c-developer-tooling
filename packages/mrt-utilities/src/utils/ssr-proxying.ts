...
const HEADER_FORMATS = ['http', 'aws'] as const;
...
/**
 * Represents a host and port test configuration
 */
interface HostAndPortTest {
  type: string | null;
  regexp: RegExp;
  isIPOrLocalhost: boolean;
  hasPort: boolean;
  ...
  constructor(headers: AWSHeaders | HTTPHeaders | IncomingHttpHeaders, format: HeaderFormat) {
    ...
    for (const [key, values] of Object.entries(headers)) {
      ...
      if (this.httpFormat) {
        ...
        // so we do not split.
        if (key === SET_COOKIE) {
          this.headers[key] = (values as string[]).slice();
        } else {
          this.headers[key] = (values as string).split(/,\s*/).map((value) => value.trim());
        }
      } else {
        this.headers[key] = (values as AWSHeaderValue[]).map((value) => value.value.trim());
      }
    }
 ...
 */
export const rfc1123 = (date: Date): string => {
  const time = [
 ...
 * @private
 */
export const cookieAsString = (cookie: ParsedCookie): string => {
 ...
 * @private
 */
const HOST_AND_PORT_TESTS: HostAndPortTest[] = [
...
export const rewriteDomain = (domain: string, appHostname: string, targetHost: string): string => {
 ...
 */
export const rewriteSetCookies = ({
  appHostname,
  setCookies,
  targetHost,
  logging = false,
}: RewriteSetCookiesParams): string[] => {
...
export const rewriteProxyResponseHeaders = ({
  ...
  appProtocol = 'https',
  logging = false,
}: RewriteProxyResponseHeadersParams): AWSHeaders | HTTPHeaders | IncomingHttpHeaders => {
  const workingHeaders = new Headers(
    headers ? ({...headers} as AWSHeaders | HTTPHeaders | IncomingHttpHeaders) : {},
    headerFormat,
  );
  ...
  if (requestUrl) {
    ...
    // value to 8192 bytes.
    const fullRequestUrl = (requestUrl.startsWith('/') ? `${targetOrigin}${requestUrl}` : requestUrl).slice(
      0,
 ...
 * @private
 */
export const X_HEADERS_TO_ADD: Record<string, string> = {
  'x-mobify': 'true',
};
 ...
 * @private
 */
export const ALLOWED_CACHING_PROXY_REQUEST_HEADERS: Record<string, boolean> = {
